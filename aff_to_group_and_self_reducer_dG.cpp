...
int max_overlap_pstns_aff_to_gr_rdcr (string* best_new_def_rna,
                                      string* best_undefined_rna,
                                      int     oligs_nmb,
                                      float   aff_min_level,
                                      float   min_dG_to_target_olig,
                                      string* target_rna,
                                      string* input_rna,
                                      int     rna_length,
                                      ...
                                      int     dna_flag             )
{
    ...
    string undefined_rna = (*best_undefined_rna);
    string new_best_undefined_rna;
    //int big_aff_oligs_nmb;
    int break_flag = 0;
    int undef_rna_update_flag;
    string new_def_rna;
...
*/
    for(int mutant_ncltd_numb = 1; mutant_ncltd_numb <= max_overlap_positions_nmb; mutant_ncltd_numb++)
    {
        ...
        for (int i = 0; i < mut_variants_nmb; i++)
        {
            undef_rna_update_flag = 0;
            for (int j = 0; j < mutant_ncltd_numb; j++)
            {
                temp_var = max_overlap_positions[ arr[0][i][j] ];

                if (undefined_rna[temp_var] != 'D' &&
                    undefined_rna[temp_var] != 'H' &&
                    undefined_rna[temp_var] != 'V' &&
                    undefined_rna[temp_var] != 'B'   )
                {
                    undef_rna_update_flag = 1;
            ...
            if ( cmplx_dG > min_dG_to_target_olig ||
                 undef_rna_update_flag == 0         )
            {
                for (int j = 0; j < mutant_ncltd_numb; j++)
                {
                    temp_var = max_overlap_positions[ arr[0][i][j] ];
                    undefined_rna[temp_var] = (*best_undefined_rna)[temp_var];
                }
                continue;
            }
            antiaff_index = 0;
            AAO_table[0].seq = new_def_rna;
            //big_aff_oligs_nmb = 0;
            ...
            if (evaluate_index > max_evaluate_index)
            {
                ...
                best_antiaff_index_update_flag = 1;
                (*best_new_def_rna) = new_def_rna;
                new_best_undefined_rna = undefined_rna;
            }
            ...
            {
                temp_var = max_overlap_positions[ arr[0][i][j] ];
                undefined_rna[temp_var] = (*best_undefined_rna)[temp_var]; //if big cycle best undefined_rna should be saved
            }
        }
    ...
    (*best_undefined_rna) = new_best_undefined_rna;
    delete [] arr[0];
    delete [] arr;
    return break_flag;
}

int first_position_of_element (int* arr,
                               int  arr_length,
                               int  element)
{
...
void self_aff_func(string* cmplx_struct_part,
                   int*    overlap_arr,
                   int     max_overlap_counter)
{
    int l = (*cmplx_struct_part).length();
    int right_brckts_nmb = 0;
    int right_brckts_positions [ l ];

    for (int i = 0; i < l; i++)
    {
        if ( (*cmplx_struct_part)[i] == ')' )
        {
            right_brckts_positions [right_brckts_nmb] = i;
            right_brckts_nmb++;
        }
    }
    int symm_ncltds_pairs_nmb = (int)(right_brckts_nmb/2);
    int symm_ncltds_pairs_pstns [2][ symm_ncltds_pairs_nmb ];

    for (int i = 0; i < symm_ncltds_pairs_nmb; i++)
    {
        symm_ncltds_pairs_pstns[0][i] = right_brckts_positions[i];
        symm_ncltds_pairs_pstns[1][i] = right_brckts_positions[right_brckts_nmb - 1 - i];
    }
    int pair_pstn,
        symm_ncltd_pstn;
    for (int i = 0; i <= symm_ncltds_pairs_pstns[0][symm_ncltds_pairs_nmb - 1]; i++)
    {
        if ( overlap_arr[i] == max_overlap_counter )
        {
            pair_pstn = first_position_of_element( symm_ncltds_pairs_pstns[0],
                                                  symm_ncltds_pairs_nmb,
                                                  i                          );
            if ( pair_pstn >= 0 )
            {
                symm_ncltd_pstn = symm_ncltds_pairs_pstns[1][pair_pstn];
                if ( overlap_arr[ symm_ncltd_pstn ] == max_overlap_counter )
                    overlap_arr[ symm_ncltd_pstn ] --;
            }
        }
    }
}
                                                ...
                                                int     dna_flag)
{
    ...
    if (AAO_table[0].aff > aff_min_level)

        self_aff_func(&(AAO_table[0].cmplx_struct_part),
                      overlap_arr,
                      max_overlap_counter);
    //(*stat_w) << endl;
...
int main()
{
    ...
    float min_dG_to_target_olig;
    getline(inp_r, temp_str);
    decimal_numb_reader(&temp_str, (int)temp_str.find(' ') + 1, &min_dG_to_target_olig);
    ...
    anti_aff_olig AAO_table [oligs_nmb + 1];//
    AAO_table[0].seq = input_rna;

    for (int i = 1; i <= oligs_nmb; i++)
    {
        getline(inp_r, AAO_table[i].seq);
        (AAO_table[i].seq).erase((AAO_table[i].seq).begin(),
                                 (AAO_table[i].seq).begin() +
                                 (AAO_table[i].seq).find(' ') + 1);
    }
    ...
    string best_undefined_rna = input_rna;
    ...
    string cmplx = target_rna + "+" + best_new_def_rna;
    float cmplx_dG = dG_reader(&cmplx);
    ofstream result;
    result.open(output_file);
    result << best_new_def_rna << endl
           << "dG (kkal/mol) = " << dG_reader(&best_new_def_rna) << endl
           << "cmplx_dG (kkal/mol) = " << cmplx_dG;
    if (cmplx_dG < min_dG_to_target_olig)
         result << " +" << endl;
    else result << " -" << endl;
    ...
    for (int i = 0; i <= oligs_nmb; i++)
    {
        ...
        result << i << ") " << aff_to_olig_from_group;
