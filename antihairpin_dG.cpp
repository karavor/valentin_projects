                   ...
                   //int loop_counter)
{
    ...
    for (int j = 0; j < 4; j++)
    {
        ...
        new_cmplx_dG = dG_reader(&complex_seq);

        antihairpin_index[j]   = (new_dG - initial_dG) / abs(initial_dG); //*bigstat_w << antihairpin_index[j] << '\t';// higher is better
        lost_affinity_index[j] = (new_cmplx_dG - initial_cmplx_dG) / abs( initial_cmplx_dG );   //*bigstat_w << lost_affinity_index[j] << '\t';// lower is better
    ...
    *lost_affinity_index_ = lost_affinity_index[i];
    ...
    getline(output_r, temp_str);
    decimal_numb_reader(&temp_str, 0, &new_cmplx_dG);
    ...
    *stat_w << new_rna_struct << endl;
    ...
    if (break_flag)
    {
        (*output_w) << new_defined_rna << '\t'
                    << new_cmplx_dG << '\t'
                    << new_dG  << '\t'
                    << aff_reader(&new_defined_rna, input_rna, dna_flag)  << '\t'
                    << '\t' << '-' << endl;
        return 1;
    }
